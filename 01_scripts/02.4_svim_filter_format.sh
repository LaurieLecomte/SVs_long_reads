...
## Extract old names
bcftools query -l $MERGED_DIR/svim/svim_PASS_refined.vcf > $MERGED_DIR/svim/svim_PASS_refined.old_names

## Extract new names
sed -E 's/[0-9]+\_([A-Za-z0-9]+)/\1/' $MERGED_DIR/svim/svim_PASS_refined.old_names > $MERGED_DIR/svim/svim_PASS_refined.new_names

## Paste old and new names in a single file to supply to bcftools reheader
paste -d "\t" $MERGED_DIR/svim/svim_PASS_refined.old_names $MERGED_DIR/svim/svim_PASS_refined.new_names > $MERGED_DIR/svim/svim_PASS_refined.old_new_names
